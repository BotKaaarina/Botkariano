...
/*------------------------------------------------*/
		...
		if (!isNumber(user.IDregister)) user.IDregister = 0   
	      ...
	      if (!('role' in user)) user.role = '*НОВИЧОК* 🪤'
...
if (m.text && user.banned && !isROwner) {
  ...
  if (user.bannedMessageCount < 3) {
    ...
    const messageText = `❰ ⚠️ ❱ *ЗАБАНЕН/А* ❰ ⚠️ ❱\nПредупреждение (${messageNumber}/3)${user.bannedReason ? `\n*Причина:* *${user.bannedReason}*` : ''}
*👉 Вы можете связаться с владелицей бота, если считаете, что это ошибка (ИМЕТЬ ДОКАЗАТЕЛЬСТВА), чтобы обсудить вопрос вашего разбана.*
...
if (botSpam.antispam && m.text && user && user.lastCommandTime && (Date.now() - user.lastCommandTime) < 5000 && !isROwner) {
  if (user.commandCount === 5) {
    ...
    if (remainingTime > 0) {
      const messageText = `*"ОСТАЛОСЬ ${remainingTime} СЕКУНД ДО ИСТЕЧЕНИЯ ВРЕМЕНИ КОМАНДЫ`
      m.reply(messageText)
      return
    } else {
      user.commandCount = 0
  ...
  user.commandCount = 1
               ...
               if (plugin.rowner && plugin.owner && !(isROwner || isOwner)) { // Both Owner
                ...
                if (!isPrems && plugin.money && global.db.data.users[m.sender].money < plugin.money * 1) {
                    this.reply(m.chat, `💰 У вас не осталось монет`, m)
                    continue     
		}
		    ...
		    continue // If the level has not been reached
                ...
                try {
                 ...
                 if (m.money)
                        m.reply(+m.money + '💰 Монет потрачено')
...
const text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Добро пожаловать, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '🪐 Чат-Менеджер 🪐') :
(chat.sBye || this.bye || conn.bye || 'Прощай, @user!')).replace('@user', '@' + user.split('@')[0]);

if (chat.antifake && isBotAdminNn && action === 'add') {
    const allowedStartNumbers = ["1", "2", "4", "6", "7", "8", "9"]; // Вы можете изменять пользователей, которые будут удалены, если начинаются с любой из этих чисел	
    if (allowedStartNumbers.some(num => user.startsWith(num))) {	
...
case 'daradmin':
case 'darpoder':
text = (chat.sPromote || this.spromote || conn.spromote || '@user ```Новый Администратор группы```')
case 'demote':
case 'quitarpoder':
case 'quitaradmin':
if (!text)
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
       ...
       // if (groupUpdate.desc) text = (chats.sDesc || this.sDesc || conn.sDesc || '```Описание было изменено на```\n@desc').replace('@desc', groupUpdate.desc);
        ...
        //if (groupUpdate.icon) text = (chats.sIcon || this.sIcon || conn.sIcon || '```Иконка была изменена на```').replace('@icon', groupUpdate.icon);
...
Здравствуйте если вы получили это сообщение не обращайте внимание это ошибка
`.trim(), msg, {
...
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: lenguajeGB.smsAvisoAG().slice(0,-2), body: [wm, '😻' + gt + ' 😻'].getRandom(), thumbnail: gataImg.getRandom(), sourceUrl: [md, nna, nn, nnn, nnnt, nnnttt4, yt, ig, paypal, fb].getRandom() }}}}, tg)
